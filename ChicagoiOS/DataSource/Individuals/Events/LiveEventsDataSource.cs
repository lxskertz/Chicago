...
{
    public class LiveEventsDataSource : UITableViewSource
    {
        ...
        /// </summary>
        private NSString LiveEventsCell = new NSString("LiveEventsCell"); 
        ...
        public LiveEventsDataSource(LiveToastersEventsController controller, List<BusinessEvents> businessEvents,
        ...
        {
            var cell = (LiveEventsCell)tableView.DequeueReusableCell(this.LiveEventsCell);
            ...
            if (item != null)
            {
                ...
                var userLikedEvent = this.Controller.EventsLikeList.Where(x => x.Key == item.EventId).FirstOrDefault();
        ...
        public async Task GetLikeEventList(List<BusinessEvents> rows)
        {
            foreach (var b in rows)
            {
                ...
                if (liked != null)
                {
                    this.Controller.AddRemoveEventLike(liked.Liked, b.EventId);
                }
            }
        }
        ...
        private async void BeginDownloadingImage(ImageViewImage logo, NSIndexPath path, UITableView tableView)
        {
            try
            {
                ...
                InvokeOnMainThread(() =>
                {
                    ...
                    if (cell != null && cell is LiveEventsCell)
                    {
                        var bcell = (LiveEventsCell)cell;
                        bcell._EventLogo.Image = logo.Image;
                    }

                    //var cell = (LiveEventsCell)tableView.VisibleCells.Where(c => c.Tag == this.ImageViewImage.IndexOf(logo)).FirstOrDefault();
                    //if (cell != null)
        ...
        private async void GetLikeCount(BusinessEvents item, NSIndexPath path, UITableView tableView)
        {
            ...
            InvokeOnMainThread(() => {
                var cell = (LiveEventsCell)tableView.VisibleCells.Where(c => c.Tag == this.BusinessEvents.IndexOf(item)).FirstOrDefault();
                if (cell != null)
                {
                    SetLikeCount(item, cell._LikeCount);
                }
